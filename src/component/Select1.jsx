...
export default function CameraButtons() {
  ...
  useEffect(() => {
    if (isAnimating) {
      // Close the animation after 2 seconds
      const timeoutId = setTimeout(() => {
        setIsAnimating(false);
      }, 2000);

      // Cleanup the timeout on component unmount or if another button is clicked
      return () => clearTimeout(timeoutId);
    }
  }, [isAnimating]);

  const cameraNumbers = Array.from({ length: 10 }, (_, index) => index + 1);
